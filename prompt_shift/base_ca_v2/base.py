...
def parse_args():
    ...
    # Essay-side prompt conditioning. Enabled by default because the rubric side
    # also carries prompt identity in score-level rubric strings.
    parser.add_argument(
    ...
    # Auxiliary rubric-score contrastive objective. This keeps the regression
    # scorer unchanged but adds supervision that pulls the fused essay-trait
    # representation toward the correct score-level rubric text.
...
class RubricFusionTraitWiseAESModel(nn.Module):
    ...
    def forward(
        ...
        if return_fused:
            return {"preds": preds, "fused": fused.reshape(bsz, num_traits, hidden)}
        return preds
...
def rubric_score_contrastive_loss(
    ...
    # CUDA autocast can make logits float16. A hard mask such as -1e9
    # overflows fp16, so do the masking and CE in fp32.
    logits = logits.float()
...
def main():
    ...
    print(f"Essay encoder includes prompt ID: {not args.exclude_prompt_id_in_essay_encoder}", flush=True)
    print(
        f"Rubric-score contrastive loss: {args.use_rubric_score_contrastive} "
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for step, batch in enumerate(progress, start=1):
            ...
            score_rubric_input_ids = batch["score_rubric_input_ids"].to(device)
            score_rubric_attention_mask = batch["score_rubric_attention_mask"].to(device)
            score_candidate_mask = batch["score_candidate_mask"].to(device)
            score_class_labels = batch["score_class_labels"].to(device)
        ...
        print(f"  regression loss : {train_reg_loss:.6f}", flush=True)
        print(f"  contrastive loss: {train_contrastive_loss:.6f}", flush=True)
