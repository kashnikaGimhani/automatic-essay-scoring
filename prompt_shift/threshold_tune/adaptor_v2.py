...
Notes
-----
This version uses a few-shot-size-specific DEV split for threshold tuning.
For k=8 it tunes thresholds on dev_8.tsv; for k=16 it tunes thresholds
on dev_16.tsv, etc. This avoids using one large shared target dev set
for every few-shot condition.
...
def parse_args():
    ...
    parser.add_argument("--split_root", type=str, required=True,
                        help="Root directory containing heldout_{prompt}/repeat_xx/dev_{k}.tsv and test.tsv.")
    ...
    parser.add_argument("--dev_file_template", type=str, default="dev_{k}.tsv",
                        help="Template for k-specific dev files inside each repeat dir. Default: dev_{k}.tsv")
    parser.add_argument("--allow_legacy_dev_fallback", action="store_true",
                        help="If dev_{k}.tsv is missing, fall back to old shared dev.tsv. Use only for legacy splits.")
...
def resolve_dev_path_for_k(repeat_dir: str, k: int, args) -> Optional[str]:
    """Return the k-specific dev file path for threshold tuning.

    Default expected file names are dev_8.tsv, dev_16.tsv, etc.
    A legacy fallback to dev.tsv is available only when explicitly enabled.
    """
    dev_filename = args.dev_file_template.format(k=k)
    dev_path = os.path.join(repeat_dir, dev_filename)

    if os.path.exists(dev_path):
        return dev_path

    legacy_dev_path = os.path.join(repeat_dir, "dev.tsv")
    if args.allow_legacy_dev_fallback and os.path.exists(legacy_dev_path):
        print(
            f"WARNING: k-specific dev file missing for k={k}: {dev_path}. "
            f"Falling back to legacy shared dev.tsv: {legacy_dev_path}",
            flush=True,
        )
...
def read_split_df(path: str, args) -> pd.DataFrame:
    ...
    for heldout_prompt in heldout_prompts:
        ...
        for repeat_dir in repeat_dirs:
            ...
            if not os.path.exists(test_path):
                print(f"Skipping {repeat_dir}: test.tsv missing -> {test_path}", flush=True)
                continue

            test_df = read_split_df(test_path, args)
            ...
            if args.include_zero_shot:
                print("\n--- zero-shot/base threshold tuning with k-specific dev sets ---", flush=True)
                ...
                for k in fewshot_sizes:
                    dev_path = resolve_dev_path_for_k(repeat_dir, k, args)
                    if dev_path is None:
                        expected = os.path.join(repeat_dir, args.dev_file_template.format(k=k))
                        print(f"Skipping zero-shot k={k}: dev file missing -> {expected}", flush=True)
                        continue

                    dev_df = read_split_df(dev_path, args)
                    print(f"\n--- zero-shot/base threshold tuning | dev_k={k} | dev_file={os.path.basename(dev_path)} ---", flush=True)

                    zero_out_dir = os.path.join(repeat_out_dir, "zero_shot", f"k_{k}")
            ...
            for k in fewshot_sizes:
                ...
                if dev_path is None:
                    expected = os.path.join(repeat_dir, args.dev_file_template.format(k=k))
                    print(f"Skipping head-only k={k}: dev file missing -> {expected}", flush=True)
                    continue
                ...
                print(f"\n--- head-only threshold tuning | k={k} | dev_file={os.path.basename(dev_path)} ---", flush=True)
