...
Architecture: trait-wise scalar regression heads applied after rubric-aware fusion.
...
Compared with the original base_aes.py:
  ...
  - The essay encoder input includes prompt ID + genre + prompt description + essay text.
  - A separate rubric encoder reads prompt-trait score-level rubric text from
    cross_attention_score_texts in asap_prompt_meta_v3.json.
  ...
  - Separate trait-wise scalar regression heads predict each trait score from its own fused representation.
  - Head-level rubric attention has been removed; rubric information enters through the main
    cross-attention fusion block only.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Ablation: omit Prompt ID/genre prefix from the essay encoder input.",
    )
...
def join_score_level_texts(score_texts: Dict[str, Any]) -> str:
    """Join nested score-level rubric texts in score order."""
    if not isinstance(score_texts, dict) or not score_texts:
        return ""

    def score_sort_key(k: Any):
        try:
            return float(k)
        except Exception:
            return str(k)
    ...
    parts = []
    for score, direction in sorted(score_texts.items(), key=lambda kv: score_sort_key(kv[0])):
        if isinstance(direction, str):
            # cross_attention_score_texts already contains prompt, genre, trait, score and direction.
            parts.append(direction.strip())
        elif isinstance(direction, dict):
            parts.append(f"Score {score}: {json.dumps(direction, ensure_ascii=False)}")
        else:
            parts.append(f"Score {score}: {direction}")
...
def build_essay_prompt_encoder_text(
    ...
    """Build the first sequence passed to the essay encoder.

    The essay encoder receives this text as sequence A and the essay as sequence B.
    Including the prompt ID gives explicit alignment with rubric texts such as
    "Prompt 2; genre=..." while the description preserves task semantics.
    """
    ...
    if not include_prompt_id:
        return description

    parts = [f"Prompt ID: {prompt_id}."]
    if genre:
...
def get_rubric_text(
    ...
    trait: str,
    prompt_text_map: Dict[str, str],
    rubric_text_field: str = "cross_attention_score_texts",
    include_source_excerpt: bool = False,
    source_excerpt_chars: int = 1200,
) -> str:
    """Return a single encoder-ready text for a prompt-trait pair.

    Supports both flat trait-level rubric text fields and nested score-level fields:
      cross_attention_score_texts -> trait -> score -> text
    """
    pmeta = prompt_meta.get(prompt_id, {})
    trait_text = normalize_trait_name_for_text(trait)

    text = ""
    field_value = pmeta.get(rubric_text_field)

    # Preferred path: nested score-level text for cross-attention.
    if isinstance(field_value, dict):
        trait_value = field_value.get(trait, "")
        if isinstance(trait_value, dict):
            joined = join_score_level_texts(trait_value)
            if joined:
                genre = pmeta.get("genre", "")
    ...
    # Fallback 1: score-level cross-attention texts.
    if not text and isinstance(pmeta.get("cross_attention_score_texts"), dict):
        ...
        joined = join_score_level_texts(score_texts)
        if joined:
            genre = pmeta.get("genre", "")
            text = f"Prompt {prompt_id}. Genre: {genre}. Trait: {trait_text}. Score-level directions: {joined}"
    ...
    if not text and isinstance(pmeta.get("trait_score_level_directions"), dict):
        ...
        if isinstance(directions, dict) and directions:
            joined = join_score_level_texts(directions)
            genre = pmeta.get("genre", "")
            text = f"Prompt {prompt_id}. Genre: {genre}. Trait: {trait_text}. Score-level directions: {joined}"
...
class SingleTraitRegressor(nn.Module):
    """One scalar regression head for one trait-specific fused representation.

    Head-level rubric attention is intentionally removed. Rubric information is
    already injected into the fused vector by the main cross-attention block.
    """
...
class TraitWiseRegressor(nn.Module):
    """Separate scalar regression head for each trait.

    Input is the flattened fused representation [B*T, H] created by the shared
    essay/rubric cross-attention block. This module reshapes it into [B, T, H]
    and sends each trait slice to a different regression head.
    """

    def __init__(self, hidden: int, num_traits: int, dropout: float):
        super().__init__()
        self.num_traits = num_traits
        self.heads = nn.ModuleList([
            SingleTraitRegressor(hidden=hidden, dropout=dropout)
            for _ in range(num_traits)
        ])

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() != 2:
            raise ValueError(f"Expected x shape [B*T, H], got {tuple(x.shape)}")
        if x.shape[0] % self.num_traits != 0:
            raise ValueError(f"First dimension {x.shape[0]} is not divisible by num_traits={self.num_traits}")

        bsz = x.shape[0] // self.num_traits
        hidden = x.shape[-1]
        x_by_trait = x.reshape(bsz, self.num_traits, hidden)  # [B, T, H]

        preds = []
        for trait_idx, head in enumerate(self.heads):
            trait_x = x_by_trait[:, trait_idx, :]  # [B, H]
            preds.append(head(trait_x))
...
class RubricFusionTraitWiseAESModel(nn.Module):
    ...
    def forward(
        ...
        if rubric_input_ids is None or rubric_attention_mask is None:
            ...
            return self.trait_regressors(fused_flat)
...
def build_optimizer(model: RubricFusionTraitWiseAESModel, args):
    ...
    for name, p in model.named_parameters():
        ...
        elif name.startswith("trait_regressors."):
...
def save_checkpoint(
    ...
    ckpt = {
        ...
        "model_architecture": "rubric_fusion_traitwise_regression_heads_base",
...
def main():
    ...
    print("Head rubric attention: removed/disabled", flush=True)
    print(f"Rubric text field: {args.rubric_text_field}", flush=True)
    print(f"Prompt ID in essay encoder: {not args.exclude_prompt_id_in_essay_encoder}", flush=True)
