...
def build_fallback_rubric_text(prompt_id: str, trait: str, prompt_text_map: Dict[str, str]) -> str:
    ...
    return (
        ...
        f"Use the trait definition and score range to judge this trait for the essay."
    )
...
def get_rubric_text(
    ...
    """Return one model-readable rubric text for a prompt-trait pair."""
    ...
    # Fallback: join separate score-level directions if full trait text is missing.
    ...
    # Fallback: build from trait_score_level_directions if available.
    ...
    # Optional short source excerpt for source-dependent prompts.
...
class RubricAwareAESDataset(Dataset):
    """AES dataset that returns essay inputs plus prompt-trait rubric inputs.

    rubric_input_ids shape per row: [num_traits, rubric_max_length]
    rubric_attention_mask shape per row: [num_traits, rubric_max_length]
    """
    ...
    def __init__(
        ...
        self.input_ids = essay_enc["input_ids"]
        self.attention_mask = essay_enc["attention_mask"]
        self.token_type_ids = essay_enc["token_type_ids"] if "token_type_ids" in essay_enc else None
        ...
        # Cache rubric tokenization by (prompt_id, trait) because it repeats heavily.
        rubric_cache: Dict[Tuple[str, str], Dict[str, torch.Tensor]] = {}
        rubric_input_rows = []
        rubric_mask_rows = []
        for pid in self.prompt_ids:
            ...
            for trait in trait_cols:
                ...
                trait_ids.append(rubric_cache[key]["input_ids"])
                trait_masks.append(rubric_cache[key]["attention_mask"])
            rubric_input_rows.append(torch.stack(trait_ids, dim=0))
            rubric_mask_rows.append(torch.stack(trait_masks, dim=0))

        self.rubric_input_ids = torch.stack(rubric_input_rows, dim=0) if n else torch.empty(0, t, rubric_max_length, dtype=torch.long)
        self.rubric_attention_mask = torch.stack(rubric_mask_rows, dim=0) if n else torch.empty(0, t, rubric_max_length, dtype=torch.long)
    ...
    def __getitem__(self, idx):
        item = {
            ...
            "rubric_input_ids": self.rubric_input_ids[idx],
            "rubric_attention_mask": self.rubric_attention_mask[idx],
