...
from utils import (
    ...
    build_meta_sgd_alpha,
    alpha_key_from_param_name,
    clone_named_params,
    extract_alpha_state,
    load_alpha_state,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Meta-LoRA adaptation with Reptile or Meta-SGD")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated encoder layer indices to adapt with LoRA. Empty = all layers.",
    )
    parser.add_argument(
        "--lora_layers_pattern",
        type=str,
        default="layer",
        help="PEFT layer pattern used with --lora_layers for RoBERTa/BERT encoders.",
    )

    # Meta-learning config.
    parser.add_argument("--meta_method", type=str, default="meta_sgd", choices=["reptile", "meta_sgd"])
    ...
    # Exact Meta-SGD config.
    parser.add_argument("--meta_outer_lr", type=float, default=1e-4)
    parser.add_argument("--meta_outer_weight_decay", type=float, default=0.0)
    parser.add_argument("--meta_outer_batch_size", type=int, default=1)
    parser.add_argument("--meta_outer_max_grad_norm", type=float, default=1.0)
    parser.add_argument("--meta_sgd_init_alpha", type=float, default=1e-3)
    parser.add_argument("--meta_first_order", action="store_true")

    # Final target adaptation config.
    parser.add_argument("--target_adapt_method", type=str, default="meta_sgd", choices=["meta_sgd", "adamw"])
    parser.add_argument("--target_meta_sgd_steps", type=int, default=1)
    parser.add_argument("--target_meta_sgd_select_by", type=str, default="qwk", choices=["qwk", "loss"])
    parser.add_argument("--target_meta_sgd_batch_size", type=int, default=8)
...
def parse_optional_layer_list(raw: str):
    raw = (raw or "").strip()
    if not raw:
        return None
...
def default_deep_lora_target_modules():
    return [
        "query",
        "key",
        "value"
...
def maybe_cuda_cleanup(device: torch.device):
    if device.type == "cuda":
...
def build_episode_loader(dataset, batch_size: int, shuffle: bool, num_workers: int = 0):
    if len(dataset) <= 0:
        raise ValueError("Cannot build an episode loader from an empty dataset.")
    return DataLoader(dataset, batch_size=min(max(1, batch_size), len(dataset)), shuffle=shuffle, num_workers=num_workers)


def compute_loader_loss(model, dataloader, device, args, params_override: Optional[OrderedDict] = None):
    total_num = None
    total_den = None
    ...
    for batch in dataloader:
        ...
        batch_loss = masked_regression_loss(
            preds=preds,
            targets=labels,
            mask=label_mask,
        ...
        batch_den = label_mask.sum().clamp(min=1.0)
        batch_num = batch_loss * batch_den
        ...
        if total_num is None:
            ...
            total_den = total_den + batch_den

    if total_num is None:
        return torch.tensor(float("nan"), device=device)
    return total_num / total_den.clamp(min=1.0)
...
def evaluate_episode(model, prompt_dataset, query_idx, score_ranges, global_trait_fallback, device, args, params_override: Optional[OrderedDict] = None):
...
def meta_sgd_adapt(
    ...
    args,
    create_graph: bool,
    start_params: Optional[OrderedDict] = None,
    num_steps: Optional[int] = None,
):
    current_params = get_trainable_named_params(model) if start_params is None else OrderedDict(start_params)
    total_steps = args.meta_inner_steps if num_steps is None else num_steps
    last_support_loss = None

    for _ in range(total_steps):
        support_loss = compute_loader_loss(
            model=model,
            dataloader=support_loader,
            device=device,
            args=args,
            params_override=current_params,
        )
        ...
        for (name, p), g in zip(current_params.items(), grads):
            alpha_tensor = alpha[alpha_key_from_param_name(name)]
            updated[name] = p - alpha_tensor * g
        current_params = updated
        last_support_loss = support_loss

    return current_params, last_support_loss


def meta_validate_reptile(
    model,
    prompt_datasets,
    val_pools,
    score_ranges,
...
def meta_validate_meta_sgd(
    ...
    for _ in range(args.meta_val_episodes):
        ...
        support_idx, query_idx = sample_support_query(val_pools[prompt_id], args.meta_support_k, args.meta_query_k, rng)
        support_ds = AESIndexedDataset(prompt_datasets[prompt_id], support_idx)
        query_ds = AESIndexedDataset(prompt_datasets[prompt_id], query_idx)
        support_loader = build_episode_loader(support_ds, args.meta_inner_batch_size, shuffle=False)
        query_loader = build_episode_loader(query_ds, args.eval_batch_size, shuffle=False)

        adapted_params, _ = meta_sgd_adapt(
            model=model,
        ...
        query_loss = compute_loader_loss(
            model=model,
            dataloader=query_loader,
            device=device,
            args=args,
        ...
        metrics["loss"] = float(query_loss.detach().item())
...
def meta_train_for_prompt_reptile(
    heldout_prompt,
    base_ckpt_dir,
    prompt_datasets,
    train_pools,
    ...
    if not lora_target_modules:
        lora_target_modules = default_deep_lora_target_modules()
    lora_layers = parse_optional_layer_list(args.lora_layers)
    ...
    for epoch in range(1, args.meta_num_epochs + 1):
        ...
        val_metrics = meta_validate_reptile(
            model=model,
    ...
    return model, None, param_counts, best_epoch, best_score


def meta_train_for_prompt_meta_sgd(
    heldout_prompt,
    base_ckpt_dir,
    prompt_datasets,
    train_pools,
    ...
    model.to(device)
    mark_only_lora_and_head_trainable(model)
    alpha = build_meta_sgd_alpha(model, init_alpha=args.meta_sgd_init_alpha).to(device)

    param_counts = count_parameters(model)
    alpha_param_count = count_parameter_dict_params(alpha)

    prompts = sorted(train_pools.keys())
    if not prompts:
        raise ValueError("No usable source prompt tasks available for meta-training")

    outer_trainable = [p for p in model.parameters() if p.requires_grad] + list(alpha.parameters())
    outer_optimizer = torch.optim.AdamW(
        outer_trainable,
        lr=args.meta_outer_lr,
        weight_decay=args.meta_outer_weight_decay,
    )
    ...
    best_epoch = -1
    early_stop_counter = 0
    create_graph = not args.meta_first_order

    for epoch in range(1, args.meta_num_epochs + 1):
        episode_qwks = []
        episode_query_losses = []
        episode_support_losses = []
        prog = tqdm(range(args.meta_episodes_per_epoch), desc=f"Meta-SGD epoch {epoch}/{args.meta_num_epochs}", leave=False)
        ...
        for epi in prog:
            ...
            adapted_params, support_loss = meta_sgd_adapt(
                model=model,
                support_loader=support_loader,
                alpha=alpha,
                device=device,
            ...
            (query_loss / max(1, args.meta_outer_batch_size)).backward()
            pending_steps += 1

            if pending_steps >= args.meta_outer_batch_size or epi == args.meta_episodes_per_epoch - 1:
                torch.nn.utils.clip_grad_norm_(outer_trainable, args.meta_outer_max_grad_norm)
                outer_optimizer.step()
                outer_optimizer.zero_grad(set_to_none=True)
                pending_steps = 0

            with torch.no_grad():
                query_metrics = evaluate(
                    model=model,
                    dataloader=query_loader,
                    dataset=query_ds,
                    trait_cols=TRAIT_COLUMNS,
            ...
            episode_qwks.append(query_metrics["mean_qwk"])
            episode_query_losses.append(float(query_loss.detach().item()))
            episode_support_losses.append(float(support_loss.detach().item()) if support_loss is not None else float("nan"))

            valid_qwk = [x for x in episode_qwks if not math.isnan(x)]
            running_qwk = float(np.mean(valid_qwk)) if valid_qwk else float("nan")
            running_loss = float(np.mean([x for x in episode_query_losses if not math.isnan(x)]))
            prog.set_postfix(qwk=f"{running_qwk:.4f}" if not math.isnan(running_qwk) else "nan", qloss=f"{running_loss:.4f}")

        val_metrics = meta_validate_meta_sgd(
            model=model,
            alpha=alpha,
            prompt_datasets=prompt_datasets,
            val_pools=val_pools,
        ...
        history.append({
            ...
            "train_episode_mean_qwk": float(np.nanmean(episode_qwks)) if episode_qwks else float("nan"),
            "train_episode_mean_support_loss": float(np.nanmean(episode_support_losses)) if episode_support_losses else float("nan"),
            "train_episode_mean_query_loss": float(np.nanmean(episode_query_losses)) if episode_query_losses else float("nan"),
            "val_mean_qwk": val_metrics["mean_qwk"],
            "val_mean_rmse": val_metrics["mean_rmse"],
            "val_mean_loss": val_metrics["mean_loss"],
        })
        ...
        if current_score > best_score:
            ...
            torch.save(
                {
                    "model_state_dict": model.state_dict(),
                    "alpha_state_dict": extract_alpha_state(alpha),
                    "best_epoch": best_epoch,
                    "best_val_score": best_score,
                    "trait_cols": trait_cols,
                    "param_counts": {
                        **param_counts,
                        "meta_sgd_alpha": alpha_param_count,
                    },
                    "meta_method": "meta_sgd",
                    "meta_config": {
                        "support_k": args.meta_support_k,
                        "query_k": args.meta_query_k,
                        "inner_steps": args.meta_inner_steps,
                        "outer_lr": args.meta_outer_lr,
                        "outer_batch_size": args.meta_outer_batch_size,
                        "first_order": args.meta_first_order,
                        "init_alpha": args.meta_sgd_init_alpha,
                    },
                    "lora_config": {
                        "r": args.lora_r,
                        "alpha": args.lora_alpha,
    ...
    if os.path.exists(best_path):
        best_state = torch.load(best_path, map_location=device)
        model.load_state_dict(best_state["model_state_dict"], strict=True)
        load_alpha_state(alpha, best_state["alpha_state_dict"], device=device)

    return model, alpha, {**param_counts, "meta_sgd_alpha": alpha_param_count}, best_epoch, best_score
...
def meta_train_for_prompt(
    ...
    if args.meta_method == "reptile":
        return meta_train_for_prompt_reptile(
    ...
    return meta_train_for_prompt_meta_sgd(
        heldout_prompt=heldout_prompt,
        base_ckpt_dir=base_ckpt_dir,
        prompt_datasets=prompt_datasets,
        train_pools=train_pools,
...
def train_target_adaptation_adamw(
    model,
    train_loader,
    dev_loader,
    dev_dataset,
...
def train_target_adaptation_meta_sgd(
    model,
    alpha,
    train_dataset,
    dev_loader,
    ...
    if alpha is None:
        raise ValueError("Meta-SGD target adaptation requires learned alpha, but alpha is None.")

    support_loader = build_episode_loader(train_dataset, args.target_meta_sgd_batch_size, shuffle=False)
    current_params = get_trainable_named_params(model)

    history = []
    best_score = -1e9
    best_step = 0
    best_params = clone_named_params(current_params)

    for step_idx in range(1, args.target_meta_sgd_steps + 1):
        current_params, support_loss = meta_sgd_adapt(
            model=model,
            support_loader=support_loader,
            alpha=alpha,
            device=device,
        ...
        history.append({
            ...
            "support_loss": float(support_loss.detach().item()) if support_loss is not None else float("nan"),
            "dev_loss": dev_metrics["loss"],
            "dev_mean_qwk": dev_metrics["mean_qwk"],
            "dev_mean_rmse": dev_metrics["mean_rmse"],
        })

        if args.target_meta_sgd_select_by == "qwk":
            current_score = dev_metrics["mean_qwk"]
            if math.isnan(current_score):
                current_score = -1e9
        else:
            current_score = -dev_metrics["loss"] if not math.isnan(dev_metrics["loss"]) else -1e9
    ...
    restore_trainable_state(model, best_params)
    best_dev_qwk = max([row["dev_mean_qwk"] for row in history if not math.isnan(row["dev_mean_qwk"])], default=float("nan"))
    ...
    torch.save(
        ...
        os.path.join(run_dir, "best_meta_sgd_target_adapt.pt"),
    )

    return model, history, best_step, best_dev_qwk
...
def build_meta_lora_model_and_alpha_from_base(base_ckpt_dir, device, args, alpha_state_dict=None):
    model, trait_cols = build_meta_lora_model_from_base(base_ckpt_dir, device, args)
    mark_only_lora_and_head_trainable(model)
    alpha = build_meta_sgd_alpha(model, init_alpha=args.meta_sgd_init_alpha).to(device)
    if alpha_state_dict is not None:
        load_alpha_state(alpha, alpha_state_dict, device=device)
    return model, trait_cols, alpha


def main():
    args = parse_args()

    if args.target_adapt_method == "meta_sgd" and args.meta_method != "meta_sgd":
        raise ValueError(
            "target_adapt_method='meta_sgd' requires meta_method='meta_sgd' so that a learned alpha is available. "
            "Use --meta_method meta_sgd or switch target adaptation to --target_adapt_method adamw."
        )
    ...
    print(f"Meta method: {args.meta_method} | Target adaptation: {args.target_adapt_method}")
    ...
    for heldout_prompt in heldout_prompts:
        ...
        meta_model, meta_alpha, param_counts, meta_best_epoch, meta_best_score = meta_train_for_prompt(
            heldout_prompt=heldout_prompt,
            base_ckpt_dir=base_ckpt_dir,
            prompt_datasets=prompt_datasets,
            train_pools=train_pools,
        ...
        best_meta_ckpt = torch.load(os.path.join(meta_out_dir, "best_meta_init.pt"), map_location="cpu")
        best_meta_alpha_state = best_meta_ckpt.get("alpha_state_dict")
        if args.meta_method == "meta_sgd" and best_meta_alpha_state is None:
            raise ValueError(
                f"Meta-SGD checkpoint for heldout prompt {heldout_prompt} is missing alpha_state_dict. "
                "Re-run meta-training or check that the correct checkpoint was loaded."
            )
        ...
        for repeat_dir in repeat_dirs:
            ...
            if args.meta_method == "meta_sgd":
                meta_init_model, meta_trait_cols, meta_init_alpha = build_meta_lora_model_and_alpha_from_base(
                    base_ckpt_dir=base_ckpt_dir,
                    device=device,
                    args=args,
                    alpha_state_dict=best_meta_alpha_state,
                )
            ...
            del meta_init_model
            if meta_init_alpha is not None:
                del meta_init_alpha
            maybe_cuda_cleanup(device)
            ...
            for k in fewshot_sizes:
                ...
                if args.meta_method == "meta_sgd":
                    model, trait_cols, alpha = build_meta_lora_model_and_alpha_from_base(
                        base_ckpt_dir=base_ckpt_dir,
                        device=device,
                        args=args,
                        alpha_state_dict=best_meta_alpha_state,
                ...
                if args.target_adapt_method == "meta_sgd":
                    model, adapt_history, best_epoch_or_step, best_dev_qwk = train_target_adaptation_meta_sgd(
                        model=model,
                        alpha=alpha,
                    ...
                    model, adapt_history, best_epoch_or_step, best_dev_qwk = train_target_adaptation_adamw(
                        model=model,
                ...
                save_json(
                    {
                        ...
                        "fewshot_k": k,
                        "meta_method": args.meta_method,
                        "target_adapt_method": args.target_adapt_method,
                        "meta_best_epoch": meta_best_epoch,
                        "meta_best_score": meta_best_score,
                        "best_adapt_epoch_or_step": best_epoch_or_step,
                        "best_dev_mean_qwk": best_dev_qwk,
                        ...
                        "meta_sgd_alpha_params": param_counts.get("meta_sgd_alpha", 0),
                    },
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(os.path.join(args.output_root, "meta_sgd_summary.csv"), index=False)
