...
Architecture: Option A — shared regressor applied trait-by-trait.
  ...
  - Rubric encoder: a separate encoder that reads prompt-trait rubric text from asap_prompt_meta_v3.json;
    frozen by default, optionally adapted with rubric-encoder LoRA during few-shot training.
...
def parse_args():
    ...
    parser.add_argument(
        "--score_rubric_encode_batch_size",
        type=int,
        default=32,
        help=(
            "Encode flattened score-level rubric candidates in small chunks. "
            "This reduces CUDA peak memory when rubric encoder LoRA is enabled. "
            "Use <=0 to encode all candidates at once."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "For the auxiliary rubric-score contrastive loss, encode score-level rubric candidates "
            "under no_grad/eval. The main prompt-trait rubric encoder path can still train rubric LoRA. "
            "This is useful when rubric_encoder_lora + contrastive loss causes CUDA OOM."
        ),
    )
    ...
    # Optional LoRA adaptation for the rubric encoder during few-shot training.
    # This keeps the architecture unchanged: the same rubric encoder is still used,
    # but only low-rank adapter weights inside it become trainable.
    parser.add_argument("--rubric_encoder_lora", action="store_true", help="Apply LoRA to the rubric encoder during few-shot adaptation.")
    parser.add_argument("--rubric_lora_r", type=int, default=4)
    parser.add_argument("--rubric_lora_alpha", type=int, default=8)
    parser.add_argument("--rubric_lora_dropout", type=float, default=0.1)
    parser.add_argument("--rubric_lora_target_modules", type=str, default="query,value")
    parser.add_argument("--rubric_lora_bias", type=str, default="none", choices=["none", "all", "lora_only"])
    parser.add_argument("--rubric_lora_lr", type=float, default=-1.0, help="Use -1 to reuse --cross_attn_lr for rubric-encoder LoRA.")
    parser.add_argument("--rubric_use_rslora", action="store_true", help="Enable rank-stabilized LoRA for the rubric encoder.")
    parser.add_argument("--rubric_use_dora", action="store_true", help="Enable DoRA for the rubric encoder.")
    parser.add_argument(
        "--rubric_encoder_gradient_checkpointing",
        action="store_true",
        help="Enable gradient checkpointing on the rubric encoder to reduce memory when rubric LoRA is trained.",
    )
...
class RubricFusionTraitWiseLoRAAESModel(nn.Module):
    ...
    def __init__(
        ...
        self.rubric_encoder_lora_enabled = False
        ...
        # Shared scalar scorer applied to every trait-specific fused representation.
    ...
    def encode_rubric(self, rubric_input_ids: torch.Tensor, rubric_attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        if self.freeze_rubric_encoder and not self.rubric_encoder_lora_enabled:
    ...
    def encode_score_rubrics(
        ...
        score_rubric_encode_batch_size: int = 32,
        no_grad_encode: bool = False,
    ) -> torch.Tensor:
        # score_rubric_input_ids: [B, T, C, R]
        #
        # With rubric_encoder_lora + contrastive loss, this tensor can become very
        # large: B * num_traits * num_score_candidates separate rubric sequences.
        # Encoding all of them in one RoBERTa forward pass can OOM on a 14-16GB GPU.
        # Chunking keeps the same architecture/objective but lowers the temporary
        # activation peak. `no_grad_encode=True` is an additional memory-safe mode
        # where score-level rubric candidates act as fixed anchors for contrastive
        # learning, while the main prompt-trait rubric encoder path can still train
        # rubric LoRA through the regression/cross-attention loss.
        ...
        chunk_size = int(score_rubric_encode_batch_size) if score_rubric_encode_batch_size is not None else 0
        if chunk_size <= 0 or chunk_size > total:
            chunk_size = total

        use_no_grad = bool(no_grad_encode or (self.freeze_rubric_encoder and not self.rubric_encoder_lora_enabled))
        was_training = self.rubric_encoder.training
        if use_no_grad:
            self.rubric_encoder.eval()
        ...
        for start in range(0, total, chunk_size):
            ...
            if use_no_grad:
                with torch.no_grad():
                    out = self.rubric_encoder(input_ids=ids_chunk, attention_mask=mask_chunk)
                    hidden = out.last_hidden_state
            else:
                out = self.rubric_encoder(input_ids=ids_chunk, attention_mask=mask_chunk)
                hidden = out.last_hidden_state
            ...
            hidden = self.rubric_projection(hidden)
            pooled = self.pool_tokens(hidden, mask_chunk, which="context")
            pooled_chunks.append(pooled)

        if use_no_grad and was_training:
            self.rubric_encoder.train()
    ...
    def forward(
        ...
        if return_fused:
            return {"preds": trait_scores, "fused": fused.reshape(bsz, num_traits, hidden)}
        return trait_scores
...
def rubric_score_contrastive_loss(
    ...
    return torch.nn.functional.cross_entropy(logits_valid, targets_valid)
...
def mark_only_lora_cross_attention_trainable(model: RubricFusionTraitWiseLoRAAESModel, freeze_shared_regressor: bool = False):
    ...
    # Shared trait-wise regressor. This replaces the old multi-output head.
    if not freeze_shared_regressor:
        for p in model.shared_regressor.parameters():
            p.requires_grad = True
    ...
    # Rubric encoder adaptation.
    # - If rubric_encoder_lora_enabled=True, train only rubric LoRA adapter weights.
    # - Else, preserve the existing optional full-unfreeze behavior.
    if getattr(model, "rubric_encoder_lora_enabled", False):
        for name, p in model.rubric_encoder.named_parameters():
            if "lora_" in name or "modules_to_save" in name:
                p.requires_grad = True
    elif not model.freeze_rubric_encoder:
...
def build_optimizer(model: nn.Module, args):
    ...
    rubric_lora_lr = args.rubric_lora_lr if args.rubric_lora_lr > 0 else cross_lr
    ...
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if name.startswith("rubric_encoder.") and (".lora_" in name or "lora_" in name):
            rubric_lora_params.append(p)
    ...
    if rubric_lora_params:
        groups.append({"params": rubric_lora_params, "lr": rubric_lora_lr, "weight_decay": args.weight_decay})
    if regressor_params:
    ...
    for epoch in range(1, args.num_epochs + 1):
            ...
            with torch.autocast(device_type=autocast_device, dtype=autocast_dtype, enabled=amp_enabled):
                ...
                if args.use_rubric_score_contrastive:
                    ...
                    score_rubric_emb = model.encode_score_rubrics(
                        ...
                        score_rubric_encode_batch_size=args.score_rubric_encode_batch_size,
                        no_grad_encode=args.freeze_score_rubric_encoder_in_contrastive,
                    )
        ...
        if improved:
            ...
            torch.save({
                ...
                "architecture": "rubric_lora_cross_attention_fusion_traitwise_rubric_attentive_head" if model.head_rubric_attention else "rubric_lora_cross_attention_fusion_traitwise_shared_regressor",
                "rubric_config": {
                    ...
                    "score_rubric_encode_batch_size": args.score_rubric_encode_batch_size,
                    "freeze_score_rubric_encoder_in_contrastive": args.freeze_score_rubric_encoder_in_contrastive,
                    ...
                    "rubric_encoder_lora": args.rubric_encoder_lora,
                ...
                "rubric_lora_config": {
                    "enabled": args.rubric_encoder_lora,
                    "r": args.rubric_lora_r,
                    "alpha": args.rubric_lora_alpha,
                    "dropout": args.rubric_lora_dropout,
                    "target_modules": [x.strip() for x in args.rubric_lora_target_modules.split(",") if x.strip()],
                    "bias": args.rubric_lora_bias,
                    "lr": args.rubric_lora_lr,
                    "use_rslora": args.rubric_use_rslora,
                    "use_dora": args.rubric_use_dora,
                },
            }, best_ckpt_path)
        else:
...
def apply_lora_to_model_encoder(
    ...
    This rubric-aware architecture has `shared_regressor`, not `regressor`, so
    LoRA should be applied only to the essay encoder.
    """
    try:
...
def apply_lora_to_rubric_encoder(
    ...
    """Apply LoRA directly to model.rubric_encoder.

    This does not change the rubric-aware architecture. It only makes low-rank
    adapter weights inside the existing rubric encoder trainable during few-shot
    adaptation. The base rubric encoder weights remain frozen by PEFT and by the
    trainability filter below.
    """
    try:
        from peft import LoraConfig, TaskType, get_peft_model
    except ImportError as exc:
    ...
    model.rubric_encoder = get_peft_model(model.rubric_encoder, lora_config)
    model.rubric_encoder_lora_enabled = True
    return model


def enable_gradient_checkpointing_if_requested(module: nn.Module, module_name: str = "model"):
    """Enable gradient checkpointing when supported by Transformers/PEFT."""
    try:
        if hasattr(module, "gradient_checkpointing_enable"):
            module.gradient_checkpointing_enable()
        elif hasattr(module, "base_model") and hasattr(module.base_model, "gradient_checkpointing_enable"):
            module.base_model.gradient_checkpointing_enable()
        else:
            print(f"Warning: {module_name} does not expose gradient_checkpointing_enable(); skipping.")
            return
        ...
        if hasattr(module, "base_model"):
            base_cfg = getattr(module.base_model, "config", None)
            if base_cfg is not None and hasattr(base_cfg, "use_cache"):
                base_cfg.use_cache = False
        print(f"Gradient checkpointing enabled for {module_name}.")
    except Exception as exc:
        print(f"Warning: could not enable gradient checkpointing for {module_name}: {exc}")
...
def load_rubric_fusion_base_checkpoint(
    ...
    important_missing = [k for k in missing if not k.startswith("rubric_projection.")]
...
def build_rubric_aware_lora_model(base_ckpt_dir, device, args, trait_cols, lora_target_modules):
    ...
    if args.rubric_encoder_lora:
        rubric_lora_target_modules = [
            x.strip() for x in args.rubric_lora_target_modules.split(",") if x.strip()
        ]
        model = apply_lora_to_rubric_encoder(
            model=model,
            r=args.rubric_lora_r,
            lora_alpha=args.rubric_lora_alpha,
            lora_dropout=args.rubric_lora_dropout,
            target_modules=rubric_lora_target_modules,
            bias=args.rubric_lora_bias,
            use_rslora=args.rubric_use_rslora,
            use_dora=args.rubric_use_dora,
        )
        print(
            "Rubric encoder LoRA enabled: training only rubric LoRA adapter weights "
            "inside the existing rubric encoder."
        )

    if args.rubric_encoder_gradient_checkpointing:
        enable_gradient_checkpointing_if_requested(model.rubric_encoder, module_name="rubric_encoder")
...
def main():
    ...
    rubric_lora_target_modules = [x.strip() for x in args.rubric_lora_target_modules.split(",") if x.strip()]
    if args.rubric_encoder_lora:
        print(
            f"Rubric encoder LoRA requested: r={args.rubric_lora_r}, "
            f"alpha={args.rubric_lora_alpha}, dropout={args.rubric_lora_dropout}, "
            f"targets={rubric_lora_target_modules}, lr={args.rubric_lora_lr if args.rubric_lora_lr > 0 else 'cross_attn_lr'}"
        )
    if args.use_rubric_score_contrastive:
        print(
            f"Score-rubric contrastive encoding: chunk_size={args.score_rubric_encode_batch_size}, "
            f"freeze_score_rubric_encoder_in_contrastive={args.freeze_score_rubric_encoder_in_contrastive}"
        )
        ...
        for repeat_dir in repeat_dirs:
            ...
            for k in fewshot_sizes:
                ...
                save_json({
                    ...
                    "rubric_encoder_lora": args.rubric_encoder_lora,
                    "rubric_lora_r": args.rubric_lora_r,
                    "rubric_lora_alpha": args.rubric_lora_alpha,
                    "rubric_lora_dropout": args.rubric_lora_dropout,
                    "rubric_lora_target_modules": [x.strip() for x in args.rubric_lora_target_modules.split(",") if x.strip()],
                    "rubric_lora_bias": args.rubric_lora_bias,
                    "rubric_lora_lr": args.rubric_lora_lr,
                    "rubric_use_dora": args.rubric_use_dora,
                    "rubric_use_rslora": args.rubric_use_rslora,
                    "rubric_model_name": args.rubric_model_name,
                    "rubric_max_length": args.rubric_max_length,
                    "rubric_text_field": args.rubric_text_field,
                    "include_source_excerpt": args.include_source_excerpt,
                    ...
                    "use_dora": args.use_dora,
                    "use_rslora": args.use_rslora,
                    "rubric_encoder_gradient_checkpointing": args.rubric_encoder_gradient_checkpointing,
                    "freeze_shared_regressor": args.freeze_shared_regressor,
