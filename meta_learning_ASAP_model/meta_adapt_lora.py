...
def parse_args() -> argparse.Namespace:
    """
    Parse command-line arguments for single-fold episodic LoRA adaptation.

    This script uses the existing train/dev/test files for one fold, and it
    performs target-domain episodic adaptation with support/query episodes.
    The outer update is first-order: it applies query gradients collected after
    support-set inner updates on the LoRA-adapted model.
    """
    parser = argparse.ArgumentParser(
        description="Episodic LoRA adaptation for small AES data using one existing fold train/dev/test split."
    )

    parser.add_argument("--folds_dir", type=str, required=True, help="Root directory containing fold subdirectories or fold files.")
    parser.add_argument("--fold_id", type=int, required=True, help="Fold number to run, for example 1 or 2 or 3.")
    parser.add_argument("--train_file_template", type=str, default="fold_{fold}/train.tsv", help="Template relative to folds_dir for the train file.")
    parser.add_argument("--dev_file_template", type=str, default="fold_{fold}/dev.tsv", help="Template relative to folds_dir for the dev file.")
    parser.add_argument("--test_file_template", type=str, default="fold_{fold}/test.tsv", help="Template relative to folds_dir for the test file.")
    parser.add_argument("--sep", type=str, default="\t", help="Column separator used in fold files. Default: tab.")
    parser.add_argument("--no_header", action="store_true", help="Use when fold files have no header row.")

    parser.add_argument("--base_model_path", type=str, required=True, help="Base/alignment checkpoint path used to rebuild the T5 encoder + regressor model.")
    parser.add_argument("--meta_checkpoint", type=str, required=True, help="Path to meta_model.pt from your FOMAML training.")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save outputs for this single fold.")

    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility.")
    parser.add_argument("--max_input_len", type=int, default=384, help="Maximum tokenized input length.")
    parser.add_argument("--eval_batch_size", type=int, default=16, help="Batch size for dev/test loaders.")
    parser.add_argument("--num_epochs", type=int, default=20, help="Maximum number of episodic adaptation epochs.")
    parser.add_argument("--lr", type=float, default=2e-4, help="Outer learning rate for LoRA adaptation.")
    parser.add_argument("--inner_lr", type=float, default=1e-3, help="Inner-loop learning rate for support-set adaptation.")
    parser.add_argument("--weight_decay", type=float, default=0.01, help="Weight decay for outer AdamW optimizer.")
    parser.add_argument("--warmup_ratio", type=float, default=0.1, help="Warmup ratio for linear scheduler.")
    parser.add_argument("--max_grad_norm", type=float, default=1.0, help="Gradient clipping norm.")
    parser.add_argument("--patience", type=int, default=5, help="Early stopping patience based on dev mean trait QWK.")

    parser.add_argument("--support_size", type=int, default=8, help="Number of support examples per task in an episode.")
    parser.add_argument("--query_size", type=int, default=16, help="Number of query examples per task in an episode.")
    parser.add_argument("--tasks_per_episode", type=int, default=4, help="Number of tasks sampled per episode.")
    parser.add_argument("--episodes_per_epoch", type=int, default=100, help="Number of episodes per training epoch.")
    parser.add_argument("--inner_steps", type=int, default=1, help="Number of inner support-set gradient steps.")
    parser.add_argument("--sample_with_replacement", action="store_true", help="Sample support/query with replacement when a task has too few examples.")

    parser.add_argument("--lora_r", type=int, default=8, help="LoRA rank.")
    parser.add_argument("--lora_alpha", type=int, default=16, help="LoRA alpha.")
    parser.add_argument("--lora_dropout", type=float, default=0.1, help="LoRA dropout.")
    parser.add_argument("--lora_target_modules", type=str, default="q,v", help="Comma-separated target modules for LoRA, e.g. q,v or q,k,v,o.")
    parser.add_argument("--train_regressor", action="store_true", help="Also fine-tune the regression head during episodic adaptation.")
...
def get_prompt_description(essay_set) -> str:
    """Return the hardcoded prompt text for a given essay set."""
...
def load_table(path: str, sep: str, no_header: bool) -> pd.DataFrame:
    """
    Read a train/dev/test table and standardize the expected column names.

    Supported no-header formats:
    - 8 columns: essay_id, essay_set, essay, ideas, flow, coherence, vocab, grammar
    - 9 columns: essay_id, essay_set, essay, ideas, flow, coherence, vocab, grammar, overall
    """
    if no_header:
        df = pd.read_csv(path, sep=sep, header=None)
        if df.shape[1] == 8:
            df.columns = ["essay_id", "essay_set", "essay", "ideas", "flow", "coherence", "vocab", "grammar"]
        elif df.shape[1] == 9:
            df.columns = ["essay_id", "essay_set", "essay", "ideas", "flow", "coherence", "vocab", "grammar", "overall_score"]
        else:
            raise ValueError(
...
def prepare_target_dataframe(
    ...
    if "overall_score" not in df_wide.columns:
        df_wide["overall_score"] = df_wide[TARGET_TRAITS].mean(axis=1).round().astype(int)
    ...
    for _, row in df_wide.iterrows():
        ...
        overall_true = int(round(float(row["overall_score"])))
...
def apply_lora(model: T5EncoderRegressor, args: argparse.Namespace) -> None:
    ...
    In this episodic adaptation script, the trainable parameters are the LoRA
    weights and optionally the custom regression head.
    """
...
def build_task_index(df_long: pd.DataFrame) -> Dict[str, np.ndarray]:
    """
    Build task pools for episodic sampling.

    Each task is defined as one (essay_set, trait) pair.
    """
    task_to_indices: Dict[str, np.ndarray] = {}
    for (essay_set, trait), part in df_long.groupby(["essay_set", "trait"]):
        task_id = f"{essay_set}::{trait}"
        task_to_indices[task_id] = part.index.to_numpy()
...
def sample_support_query_indices(
    ...
    """
    Sample support and query examples for one task.

    If the task has too few examples and replacement is enabled, the function
    samples with replacement. Otherwise it raises an error.
    """
    total_needed = support_size + query_size

    if len(indices) >= total_needed:
        sampled = rng.choice(indices, size=total_needed, replace=False)
        support_idx = sampled[:support_size]
        query_idx = sampled[support_size:]
        return support_idx, query_idx

    if not sample_with_replacement:
        raise ValueError(
            f"Task only has {len(indices)} examples, but support_size + query_size = {total_needed}. "
            "Use --sample_with_replacement or reduce support/query sizes."
        )

    support_idx = rng.choice(indices, size=support_size, replace=True)
    query_idx = rng.choice(indices, size=query_size, replace=True)
    return support_idx, query_idx
...
def collate_records(
    ...
    """Tokenize a list of records into a single batch dictionary."""
    texts = [x["input_text"] for x in records]
    enc = tokenizer(
        texts,
        return_tensors="pt",
        padding=True,
    ...
    return {
        ...
        "scores": torch.tensor([x["score_norm"] for x in records], dtype=torch.float32, device=device).unsqueeze(-1),
        "meta": records,
    }


def get_trainable_params(model: T5EncoderRegressor) -> List[torch.nn.Parameter]:
    """Return the list of trainable parameters for the outer and inner updates."""
    return [p for p in model.parameters() if p.requires_grad]


def clone_trainable_params(trainable_params: List[torch.nn.Parameter]) -> List[torch.Tensor]:
    """Clone the current trainable parameter values so they can be restored after a task."""
    return [p.detach().clone() for p in trainable_params]


def restore_trainable_params(trainable_params: List[torch.nn.Parameter], saved_params: List[torch.Tensor]) -> None:
    """Restore trainable parameters in-place from cloned tensors."""
    with torch.no_grad():
        for p, saved in zip(trainable_params, saved_params):
            p.copy_(saved)


def inner_update_on_support(
    ...
    """
    Perform inner-loop support-set updates directly on the current trainable params.

    This uses simple gradient descent in-place on the LoRA weights (and the
    regression head if enabled).
    """
    last_support_loss = 0.0
    for _ in range(inner_steps):
        support_preds = model(support_batch["input_ids"], support_batch["attention_mask"])
        support_loss = compute_loss(support_preds, support_batch["scores"])
        ...
        with torch.no_grad():
            for p, g in zip(trainable_params, grads):
                if g is not None:
                    p.add_(g, alpha=-inner_lr)
        last_support_loss = float(support_loss.item())
    return last_support_loss


def run_episodic_epoch(
    model: T5EncoderRegressor,
    train_df_long: pd.DataFrame,
    task_to_indices: Dict[str, np.ndarray],
    tokenizer: T5Tokenizer,
    ...
    """
    Run one full episodic training epoch.

    For each episode:
    1. Sample tasks defined as (essay_set, trait)
    2. For each task, sample support/query examples
    3. Do inner-loop support adaptation on the trainable LoRA parameters
    4. Compute query gradients at the adapted point
    5. Restore original parameters
    6. Average query gradients across tasks and apply one outer update

    This is a first-order episodic adaptation loop. It does not backpropagate
    through the inner update itself.
    """
    model.train()
    trainable_params = get_trainable_params(model)
    if len(trainable_params) == 0:
        raise RuntimeError("No trainable parameters found. Check LoRA setup and --train_regressor.")

    available_tasks = list(task_to_indices.keys())
    if len(available_tasks) == 0:
        raise RuntimeError("No episodic tasks were created from the training split.")

    mean_support_losses = []
    mean_query_losses = []

    for _ in range(args.episodes_per_epoch):
        replace_tasks = len(available_tasks) < args.tasks_per_episode
        sampled_tasks = rng.choice(available_tasks, size=args.tasks_per_episode, replace=replace_tasks)

        query_grads_accum = [torch.zeros_like(p) for p in trainable_params]
        task_support_losses = []
        task_query_losses = []

        for task_id in sampled_tasks:
            task_indices = task_to_indices[str(task_id)]
            support_idx, query_idx = sample_support_query_indices(
                task_indices,
                args.support_size,
                args.query_size,
                rng,
            ...
            support_records = train_df_long.loc[support_idx].to_dict("records")
            query_records = train_df_long.loc[query_idx].to_dict("records")
            support_batch = collate_records(support_records, tokenizer, args.max_input_len, device)
            query_batch = collate_records(query_records, tokenizer, args.max_input_len, device)

            saved_params = clone_trainable_params(trainable_params)
            support_loss = inner_update_on_support(
                model=model,
            ...
            query_preds = model(query_batch["input_ids"], query_batch["attention_mask"])
            query_loss = compute_loss(query_preds, query_batch["scores"])
            ...
            for i, grad in enumerate(query_grads):
                if grad is not None:
                    query_grads_accum[i].add_(grad.detach())

            task_support_losses.append(support_loss)
            task_query_losses.append(float(query_loss.item()))
            restore_trainable_params(trainable_params, saved_params)

        optimizer.zero_grad()
        num_tasks = max(len(sampled_tasks), 1)
        for p, grad_sum in zip(trainable_params, query_grads_accum):
            p.grad = grad_sum / num_tasks
        torch.nn.utils.clip_grad_norm_(trainable_params, args.max_grad_norm)
        optimizer.step()
        scheduler.step()

        mean_support_losses.append(float(np.mean(task_support_losses)))
        mean_query_losses.append(float(np.mean(task_query_losses)))

    return {
        "support_loss": float(np.mean(mean_support_losses)) if mean_support_losses else 0.0,
        "query_loss": float(np.mean(mean_query_losses)) if mean_query_losses else 0.0,
    }
...
def evaluate(model: T5EncoderRegressor, data_loader: DataLoader) -> Tuple[Dict, pd.DataFrame]:
    ...
    Returned metrics include loss, MAE / RMSE, trait-wise QWK, mean trait QWK,
    and overall QWK.
    """
    model.eval()
    losses = []
...
def run_single_fold(
    ...
    """
    Train and evaluate episodic LoRA adaptation on one selected fold.
    ...
    4. Applies LoRA
    5. Runs support/query episodic adaptation on the target train split
    ...
    required_basic = ["essay_id", "essay_set", "essay"] + TARGET_TRAITS
    for split_name, df in [("train", train_wide), ("dev", dev_wide), ("test", test_wide)]:
        missing = [c for c in required_basic if c not in df.columns]
        if missing:
            raise ValueError(f"{split_name} split missing required columns {missing}. Found columns: {df.columns.tolist()}")
    ...
    task_to_indices = build_task_index(train_long)
    dev_loader = build_dataloader(dev_long, tokenizer, args.eval_batch_size, args.max_input_len, device, shuffle=False)
    test_loader = build_dataloader(test_long, tokenizer, args.eval_batch_size, args.max_input_len, device, shuffle=False)
    ...
    total_steps = max(args.episodes_per_epoch * args.num_epochs, 1)
    ...
    best_dev_qwk = -1e9
    patience_counter = 0
    rng = np.random.default_rng(args.seed + args.fold_id)

    for epoch in range(1, args.num_epochs + 1):
        train_metrics = run_episodic_epoch(
            model=model,
            train_df_long=train_long,
        ...
        epoch_record = {
            "epoch": epoch,
            "train_support_loss": float(train_metrics["support_loss"]),
            "train_query_loss": float(train_metrics["query_loss"]),
...
def main() -> None:
    """Run episodic LoRA adaptation for one selected fold only and save the outputs."""
    args = parse_args()
    ...
    result = run_single_fold(args=args, tokenizer=tokenizer, device=device)
